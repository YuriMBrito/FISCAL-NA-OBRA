...
export class BoletimModule {
  ...
  _abrirCrudItemBM(modo, itemId) {
    ...
    window._bmToggleAgregador = (isAgr) => {
      ...
      // Captura valores digitados antes de re-renderizar para não zerá-los
      const g = id => document.getElementById(id);
      const itemAtual = {
        ...(item || {}),
        id:     (g('bm-item-id')?.value   || item?.id   || ''),
        desc:   (g('bm-item-desc')?.value || item?.desc || ''),
        und:    (g('bm-item-und')?.value  || item?.und  || ''),
        qtd:    parseFloat((g('bm-item-qtd')?.value  || '').replace(',','.')) || item?.qtd || 0,
        up:     parseFloat((g('bm-item-up')?.value   || '').replace(',','.')) || item?.up  || 0,
        cod:    (g('bm-item-cod')?.value   || item?.cod   || ''),
        banco:  (g('bm-item-banco')?.value || item?.banco || ''),
        upRef:  parseFloat((g('bm-item-upref')?.value || '').replace(',','.')) || item?.upRef || 0,
        tipoBdi:(g('bm-item-tipobdi')?.value || item?.tipoBdi || ''),
      };
      if (campos) campos.innerHTML = this._formCamposItem(itemAtual, isAgr);
