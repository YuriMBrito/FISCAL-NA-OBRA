 ...
 */
function validarLimitesAditivo({ valorOriginalContrato, totalAcrescimos, totalSupressoes, tipoContrato = 'obras' }) {
  const erros = [];
 ...
 */
function validarCapMedicao({ qtdContratada, qtdAcumuladaAntes, qtdMedicaoAtual }) {
  const erros = [];
 ...
 * @returns {{ ok: boolean, erros: string[] }}
 */
function validarProrrogacao({ dataTerminoAtual, novaDataTermino, justificativa }) {
  const erros = [];
...
module.exports = { validarLimitesAditivo, validarCapMedicao, validarCNPJ, validarProrrogacao };
